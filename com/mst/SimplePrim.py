...
class SimeplePrim:
    ...
    def __init__(self, graph):
        ...
        self.vertex_notInTree = [i for i in range(1, len(self.graph.vertex))]
        self.dist = [sys.maxsize]*len(self.graph.vertex)
        self.edge_parent = [-1]*len(self.graph.vertex)      # 存放父亲节点
    ...
    def createMST(self):
        ...
        while self.vertex_notInTree:
            ...
            lastVtIntree = self.graph.vertex[i]
            for j in self.vertex_notInTree:
                ptNotIntree = self.graph.vertex[j]
            ...
            # 2. Scan 扫描dist，找到权值最短的且顶点不在tree中的点加入到tree中
            minEdge = sys.maxsize
            addVtIndex = -1
            for j in self.vertex_notInTree:
